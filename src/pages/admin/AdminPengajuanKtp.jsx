...
const JENIS_LABELS = { baru: 'KTP Baru', perpanjangan: 'Perpanjangan KTP', hilang: 'KTP Hilang' };
...
const AdminPengajuanKtp = () => {
  ...
  const handleOpenDocument = async (document) => {
    ...
    try {
      ...
      setDocumentPreviewType(contentType);
    } catch (err) {
  ...
  const getJenisLabel = (s) => JENIS_LABELS[s] || s || '-';
    ...
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          ...
          <p className="font-body-md text-on-surface-variant mt-1">Kelola pengajuan KTP masyarakat.</p>
        </div>
        <button type="button" onClick={() => fetchKtp(currentPage)} disabled={loading}
          className="inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-outline-variant/40 text-on-surface-variant hover:bg-primary/5 hover:text-primary disabled:opacity-50">
