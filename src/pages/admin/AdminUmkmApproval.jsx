  ...
  */
  const tabData = useMemo(() => {
    if (activeTab === 'menunggu') {
      return allUmkm.filter((i) => ['menunggu_verifikasi', 'diproses'].includes(i.status));
    }
...
{showRejectModal && rejectTarget && (
  ...
  >
    <div
      className="w-[500px] max-w-full bg-surface-container-lowest rounded-2xl shadow-2xl overflow-hidden flex flex-col"
      style={{ maxHeight: '90vh', minHeight: '320px' }}
      onClick={(e) => e.stopPropagation()}
    >
      {/* HEADER */}
      <div className="px-6 py-5 border-b border-outline-variant/20 flex items-start justify-between gap-4 shrink-0">
        ...
        <button
          ...
          onClick={handleCloseReject}
          disabled={Boolean(actionLoading)}
          className="w-9 h-9 shrink-0 rounded-full hover:bg-primary/10 flex items-center justify-center text-on-surface-variant disabled:opacity-50"
        >
          <span className="material-symbols-outlined">close</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <div className="rounded-xl bg-surface-container-low p-4 mb-5">
          ...
          <p className="font-label-md font-semibold text-on-surface mt-1 break-words">
        ...
        <textarea
          ...
          disabled={Boolean(actionLoading)}
          className={`block w-full px-4 py-3 rounded-xl border bg-surface outline-none resize-none transition-colors ${
            rejectError
        ...
        <div className="flex items-center justify-between gap-3 mt-1.5">
          {rejectError ? (
            <p className="text-xs text-red-600 break-words">{rejectError}</p>
          ) : (
            <span className="text-xs text-on-surface-variant">
      ...
      {/* FOOTER */}
      <div className="px-6 py-4 border-t border-outline-variant/20 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 shrink-0">
        <button
          type="button"
          onClick={handleCloseReject}
          disabled={Boolean(actionLoading)}
          className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-outline-variant text-on-surface-variant hover:bg-surface-container-low transition-colors text-sm disabled:opacity-50"
        >
          Batal
        </button>
        <button
          ...
          disabled={Boolean(actionLoading)}
          className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-error text-white hover:bg-error/90 transition-colors text-sm disabled:opacity-50"
        >
