...
const SuperAdminPengajuanSku = () => {
  ...
  const fetchSku = async (page = 1) => {
    setLoading(true); setError('');
    try {
      const response = await api.get('/admin/pengajuan/sku', { params: { page, per_page: PAGE_SIZE } });
      ...
      setPagination({
        ...
        per_page: paginator?.per_page || PAGE_SIZE,
        total: paginator?.total || data.length || 0,
      });
      setCurrentPage(paginator?.current_page || page);
    } catch (err) {
      setError(err.response?.data?.message || 'Gagal mengambil pengajuan SKU.');
  ...
  useEffect(() => {
    return () => { if (documentPreviewUrl) window.URL.revokeObjectURL(documentPreviewUrl); };
  }, [documentPreviewUrl]);

  const processedData = useMemo(
    () => skuList.filter((i) => i.status === 'menunggu_verifikasi' || i.status === 'diproses'),
    [skuList]
  );
  const completedData = useMemo(
    () => skuList.filter((i) => i.status === 'disetujui' || i.status === 'ditolak'),
    [skuList]
  );
  ...
  const handleViewDetail = async (id) => {
    ...
    try {
      const response = await api.get(`/admin/pengajuan/sku/${id}`);
      setSelected(response.data?.data || null);
    } catch (err) {
      setError(err.response?.data?.message || 'Detail pengajuan SKU gagal diambil.');
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      setSuccess(response.data?.message || 'Status pengajuan SKU berhasil diperbarui.');
      setStatusModal(null); setCatatanAdmin('');
      if (updated && selected?.id === updated.id) setSelected(updated);
      await fetchSku(currentPage);
    } catch (err) {
      const v = err.response?.data?.errors;
      setError(v ? Object.values(v).flat().find(Boolean) : err.response?.data?.message || 'Status pengajuan SKU gagal diperbarui.');
  ...
  const getDocumentLabel = (j) => ({ ktp: 'Kartu Tanda Penduduk', kk: 'Kartu Keluarga', foto_tempat_usaha: 'Foto Tempat Usaha' }[j] || j || '-');
  ...
  const isExpired = (item) => item?.status === 'disetujui' && (item?.is_expired === true || item?.eligibility === 'expired');
  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
        ...
        <button type="button" onClick={() => fetchSku(pagination.current_page)} disabled={loading}
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
          <div className="relative w-full sm:w-64">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Cari pemohon, nama usaha, atau ID..."
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-on-surface-variant">
                    {searchQuery ? `Tidak ada hasil untuk "${searchQuery}".` : 'Belum ada pengajuan SKU pada kategori ini.'}
                  </td>
                </tr>
              ) : (
                filteredData.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-outline-variant/10 hover:bg-primary/5 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="font-label-md font-semibold text-on-surface">{item.nama_usaha}</p>
                        <p className="font-label-sm text-on-surface-variant mt-1">{item.jenis_usaha || '-'}</p>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {activeTab === 'diproses' && item.status === 'menunggu_verifikasi' && (
                            <button type="button" onClick={() => openStatusModal(item, 'diproses')}
                              className="px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm">Proses</button>
                          )}
                          {activeTab === 'diproses' && (item.status === 'diproses' || item.status === 'menunggu_verifikasi') && (
                            <>
                              <button type="button" onClick={() => openStatusModal(item, 'disetujui')}
                                className="px-3 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 text-sm">Setujui</button>
                              <button type="button" onClick={() => openStatusModal(item, 'ditolak')}
                                className="px-3 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 text-sm">Tolak</button>
                            </>
                          )}
                        </div>
                      </td>
          ...
          <div className="px-6 py-4 border-t border-outline-variant/20 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button type="button" disabled={pagination.current_page === 1}
                ...
                className="px-3 py-2 rounded-lg border border-outline-variant/40 text-sm disabled:opacity-40">Sebelumnya</button>
              <button type="button" disabled={pagination.current_page === pagination.last_page}
                onClick={() => handlePageChange(pagination.current_page + 1)}
                className="px-3 py-2 rounded-lg border border-outline-variant/40 text-sm disabled:opacity-40">Berikutnya</button>
            </div>
          </div>
        )}
      </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <div className="px-6 py-5 border-b border-outline-variant/20 flex items-center justify-between shrink-0">
              <div>
                ...
                <h2 className="font-headline-md text-on-surface mt-1">Detail Pengajuan SKU</h2>
              </div>
              ...
              <>
                <div className="p-6 overflow-y-auto min-h-0 space-y-6">
                    ...
                    {isExpired(selected) && (
                      <div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="font-label-md font-semibold text-red-700">Pengajuan SKU kedaluwarsa</p>
                            <p className="text-sm text-red-700/80 mt-1">Masa berlaku pengajuan ini telah berakhir.</p>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <InfoRow label="Nama Usaha" value={selected.nama_usaha} />
                      <InfoRow label="Jenis Usaha" value={selected.jenis_usaha} />
                      <InfoRow label="Lama Menjalankan Usaha" value={selected.lama_menjalankan_usaha !== null ? `${selected.lama_menjalankan_usaha} tahun` : '-'} />
                      <InfoRow label="Perkiraan Penghasilan" value={selected.perkiraan_penghasilan_per_bulan} />
                      <div className="md:col-span-2">
                        <InfoRow label="Deskripsi Usaha" value={selected.deskripsi_usaha} />
                      </div>
                      <div className="md:col-span-2">
                        <InfoRow label="Alamat Usaha" value={`${selected.alamat_usaha || '-'} RT ${selected.rt_usaha || '-'} / RW ${selected.rw_usaha || '-'}`} />
                      </div>
                    </div>
                  </section>
      ...
      {statusModal && (
        <div className="fixed inset-0 z-60 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <div className="p-6">
              <label htmlFor="catatanAdminSku" className="block font-label-md font-semibold text-on-surface mb-2">
                Catatan Admin {statusModal.status === 'ditolak' && <span className="text-red-600">*</span>}
              </label>
              <textarea id="catatanAdminSku" value={catatanAdmin} onChange={(e) => setCatatanAdmin(e.target.value)}
              ...
              <div className="flex items-center gap-2">
                <button type="button"
                  onClick={() => {
                    ...
                    a.download = documentPreviewName || 'dokumen';
            ...
            <div className="flex-1 min-h-0 bg-neutral-200 p-3">
              {documentLoading ? (
                <div className="h-full flex flex-col items-center justify-center text-on-surface-variant">
                  <span className="material-symbols-outlined animate-spin text-primary text-4xl">progress_activity</span>
                  <p className="mt-3">Membuka dokumen...</p>
                </div>
              ) : documentPreviewUrl ? (
